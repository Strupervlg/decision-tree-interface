function toBlock(rootNode) {
    if (!rootNode.isBlock && rootNode.stmt != null) {
        ...
        blockNodeToBlock(rootNode.block);
    }
}

function blockNodeToBlock(blockNode) {
    ...
    while (current != null) {
        ...
        stmtBlock = stmtNodeToBlock(current);
...
function stmtNodeToBlock(stmtNode) {
    if (stmtNode.secondExpr == null) {
        return printExprNode(stmtNode.firstExpr);
    ...
    switch (exprNode.type) {
        ...
        case ExprType.CAST:
            ...
            if (exprNode.cast.type == ExprType.CLASS) {
                ...
                var classBlock = printExprNode(exprNode.cast);
            }
            checkTypeBlocks(resBlock, classBlock, "class");
            resBlock.getInput("class").connection.connect(classBlock.outputConnection);
            return resBlock
